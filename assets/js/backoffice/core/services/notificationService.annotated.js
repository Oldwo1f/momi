angular.module('core').factory('notificationService',["$compile", "$sailsSocket", "$q", "$http", "$state", function ($compile,$sailsSocket,$q,$http,$state) {
    ...
    service.fetchLast= function(sort,page,nbPerPage) {
        var deferred = $q.defer();
        $sailsSocket.get('/api/notification?sort=createdAt DESC&limit=100').success(function (data,status) {
            deferred.resolve(data);
        }).error(function (data,status) {
